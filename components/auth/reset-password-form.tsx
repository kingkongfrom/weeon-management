...
function SetPasswordCard({ token }: { token: string }) {
  const [state, formAction] = useActionState(completePasswordReset, null);
  ...
  return (
    <div className="login-card rounded-2xl p-6 sm:p-8">
      ...
      <p className="mt-2 text-sm">
        Choose a strong password for your Weeon Ops console account.
      </p>
...
function ResetController() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (!token) {
        setState({ status: "error", error: "This reset link is invalid or missing." });
        return;
      }
      const res = await validateResetToken(token);
      if (!res.ok) {
        setState({ status: "error", error: res.error ?? "This reset link is invalid or expired." });
        return;
      }
    ...
    return (
      <AmbientPage>
        <div className="login-card rounded-2xl p-6 text-center sm:p-8">
          <h1 className="text-2xl font-bold tracking-tight">Reset link invalid</h1>
